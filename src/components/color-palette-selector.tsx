...
function useDebouncedSave(delay: number = 3000) {
  ...
  const debouncedSave = useCallback(
    async (currentPalette: ColorPaletteKey, isDarkMode: boolean) => {
...
export const colorPalettes = {
  murder: {
    ...
    get muted() {
      return createMutedColor(this.text, false);
    }, // muted version of text
...
export function ColorPaletteSelector({
  ...
  const { debouncedSave, isSaving } = useDebouncedSave(1000);
  ...
  const handlePaletteSelect = useCallback(
    (palette: ColorPaletteKey) => {
      ...
      debouncedSave(palette, isDarkMode);
      setIsCollapsed(true);
      setIsOpen(false);
      setIsMobileMenuOpen(false);
    },
    [onPaletteChange, debouncedSave, isDarkMode],
  );

  const toggleDarkMode = useCallback(() => {
    const newIsDarkMode = !isDarkMode;
    setIsDarkMode(newIsDarkMode);
    debouncedSave(currentPalette, newIsDarkMode);
  }, [isDarkMode, setIsDarkMode, currentPalette, debouncedSave]);
  ...
  const handleCVClick = useCallback((e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    window.open(
      "https://www.self.so/sean-nyandusi",
      "_blank",
      "noopener,noreferrer",
    );
  }, []);
                    ...
                    >
                      {isCVHovered ? (
                        <ExternalLink
                          ...
                          target="_blank"
                          href="https://www.self.so/sean-nyandusi"
                        />
                      ) : (
                        <FileText
                          className="w-4 h-4"
          ...
          <DialogTrigger asChild>
            ...
            >
              {isCVHovered ? (
                <ExternalLink
                  className="w-5 h-5"
                  style={{ color: isDarkMode ? "#f8f8f8" : "#374151" }}
                />
              ) : (
                <FileText
                  className="w-5 h-5"
...
function CVViewer() {
  ...
  const handleViewInNewTab = useCallback(() => {
    window.open(
      "/Sean_Motanya_3_Years_Experience_Software_Developer_Designer_2025.pdf",
      "_blank",
      "noopener,noreferrer",
    );
  }, []);
  ...
  const cvPreviewUrl =
    "https://pub-c6a134c8e1fd4881a475bf80bc0717ba.r2.dev/Sean_Motanya_3_Years_Experience_Software_Developer_Designer_2025.pdf";
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex ${isMobile ? "flex-col gap-3" : "justify-center"} pt-4`}
      >
        {isMobile && (
          <Button
